...
ParticleBarianSkillMesh::ParticleBarianSkillMesh(ID3D11Device *pd3dDevice) : CMesh(pd3dDevice)
{
	m_Texture = 0;
...
ParticleBarianSkillMesh::~ParticleBarianSkillMesh(void)
{
}

void ParticleBarianSkillMesh::Render(ID3D11DeviceContext *pd3dDeviceContext)
{
...
void ParticleBarianSkillMesh::CreateRasterizerState(ID3D11Device *pd3dDevice)
{
...
void ParticleBarianSkillMesh::RenderInstanced(ID3D11DeviceContext *pd3dDeviceContext, int nInstances, int nStartInstance)
{
...
bool ParticleBarianSkillMesh::Initialize(ID3D11Device* pd3dDevice, WCHAR* textureFilename)
{
	bool result;
...
bool ParticleBarianSkillMesh::InitializeBuffers(ID3D11Device* pd3dDevice)
{
...
bool ParticleBarianSkillMesh::LoadTexture(ID3D11Device* pd3dDevice, WCHAR* filename)
{
	bool result;
...
void ParticleBarianSkillMesh::ReleaseTexture()
{
	if(m_Texture)
	{
		m_Texture->Release();
...
ID3D11ShaderResourceView* ParticleBarianSkillMesh::GetTexture()
{
	return m_Texture->GetTexture();
}


int ParticleBarianSkillMesh::GetIndexCount()
{
	return m_nIndices;
}

bool ParticleBarianSkillMesh::InitializeParticleSystem()
{
	int i;

	m_particleDeviationX = 20.f;
	m_particleDeviationY = 10.f;
	m_particleDeviationZ = 20.f;
...
void ParticleBarianSkillMesh::EmitParticles(float frameTime)
{
...
void ParticleBarianSkillMesh::UpdateParticles(float frameTime)
{
	int i;
	...
	for(i=0; i<m_currentParticleCount; i++)
	{
		...
		//m_particleList[i].positionX += frameTime * 10;//(((float)rand()-(float)rand())/RAND_MAX) * frameTime * 10;
		//m_particleList[i].positionY += (rand()%2 - 1) * frameTime * 10;//(((float)rand()-(float)rand())/RAND_MAX) * frameTime * 10;
		//m_particleList[i].positionZ += (rand()%2 - 1) * frameTime * 10;//(((float)rand()-(float)rand())/RAND_MAX) * frameTime * 10;
...
void ParticleBarianSkillMesh::KillParticles()
{
	int i, j;
	...
	for(i=0; i<m_maxParticles; i++)
	{
		if((m_particleList[i].active == true) && (m_particleList[i].positionY > 20.0f))//(m_particleList[i].m_fTime > 5.0f))
		{
...
bool ParticleBarianSkillMesh::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	int index, i;
	HRESULT result;
...
void ParticleBarianSkillMesh::Frame(float frameTime)
{
	//bool result;
...
void ParticleBarianSkillMesh::SetCamVec(D3DXVECTOR3 _vCamRight, D3DXVECTOR3 _vCamUp)
{
