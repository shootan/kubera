...
ParticleMesh::ParticleMesh(ID3D11Device *pd3dDevice) : CMesh(pd3dDevice)
{
	m_Texture = 0;
...
void ParticleMesh::Render(ID3D11DeviceContext *pd3dDeviceContext)
{
...
void ParticleMesh::CreateRasterizerState(ID3D11Device *pd3dDevice)
{
...
void ParticleMesh::RenderInstanced(ID3D11DeviceContext *pd3dDeviceContext, int nInstances, int nStartInstance)
{
...
bool ParticleMesh::Initialize(ID3D11Device* pd3dDevice, WCHAR* textureFilename)
{
	bool result;
...
bool ParticleMesh::InitializeBuffers(ID3D11Device* pd3dDevice)
{
...
bool ParticleMesh::LoadTexture(ID3D11Device* pd3dDevice, WCHAR* filename)
{
	bool result;
...
void ParticleMesh::ReleaseTexture()
{
	if(m_Texture)
	{
		m_Texture->Release();
...
ID3D11ShaderResourceView* ParticleMesh::GetTexture()
{
	return m_Texture->GetTexture();
}


int ParticleMesh::GetIndexCount()
{
	return m_nIndices;
}

bool ParticleMesh::InitializeParticleSystem()
{
	int i;
...
void ParticleMesh::EmitParticles(float frameTime)
{
	...
	m_accumulatedTime += frameTime * 1000.f;
...
void ParticleMesh::UpdateParticles(float frameTime)
{
	int i;
...
void ParticleMesh::KillParticles()
{
	int i, j;
	...
	for(i=0; i<m_maxParticles; i++)
	{
		if((m_particleList[i].active == true) && (m_particleList[i].positionY < 0.0f))
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;
...
bool ParticleMesh::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	int index, i;
	HRESULT result;
...
void ParticleMesh::Frame(float frameTime)
{
	//bool result;
...
void ParticleMesh::SetCamVec(D3DXVECTOR3 _vCamRight, D3DXVECTOR3 _vCamUp)
{
