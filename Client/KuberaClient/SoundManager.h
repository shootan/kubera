...
public:
	...
	void sndLoad()
	{
		...
		result = fsystem->createSound("Sound/골렘공격.wav", FMOD_DEFAULT, 0, &GolemAttack);
		ERRCHECK(result);

		result = fsystem->createSound("Sound/몬스터공격.mp3", FMOD_DEFAULT, 0, &MonsterAttack);
		ERRCHECK(result);

		result = fsystem->createSound("Sound/승리.mp3", FMOD_DEFAULT, 0, &Win);
		ERRCHECK(result);

		result = fsystem->createSound("Sound/패배.mp3", FMOD_DEFAULT, 0, &Defeat);
		ERRCHECK(result);

		result = fsystem->createSound("Sound/타워공격.wav", FMOD_DEFAULT, 0, &TowerAttack);
		ERRCHECK(result);

		result = fsystem->createSound("Sound/넥서스파괴.wav", FMOD_DEFAULT, 0, &NexusDestroy);
		ERRCHECK(result);

		result = fsystem->createSound("Sound/타워파괴.wav", FMOD_DEFAULT, 0, &TowerDestroy);
		ERRCHECK(result);
